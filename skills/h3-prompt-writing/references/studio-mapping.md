...
2. **字幕声明**：不再用旧式末尾中文指令；字幕已由本 skill **内嵌在 `detailed_description` 对白处**（英文双引号 on-screen text 格式，见 §1 映射表与 SKILL.md「字幕（唯一权威版）」）。片型 skill **只引用该节、不另立简化版**；字幕不受档位限制，是否带字幕由用户/脚本需求决定；
...
| 六段式 | fast / quality | 末尾中文指令（任何位置） | ❌ 不烧 |
| 六段式 | fast | 内嵌英文 on-screen 声明 | ⚠️ 烧但错字（4 步字准差） |
| 六段式 | balanced | 内嵌英文 on-screen 声明 | ❌ **整段未烧** |
| **六段式** | **quality** | **内嵌英文 on-screen 声明** | ✅ **正确（逐字一致）** |

画面内英文标牌（`"Moon Base 7"`）：`quality` 与 `balanced` 两档**都逐字一致**——档位差异是**字幕（对白联动）**特有的，不适用于所有画面内文字。

**定稿规则**（权威版见 SKILL.md「字幕（唯一权威版）」，本节只记实验事实）：

1. 六段式下字幕以英文双引号 on-screen text 声明内嵌 `detailed_description`（对白处），中文措辞指令（任何位置）不生效；
2. 四条限定词缺一不可：声明紧贴对白句 / `reading exactly "原文"` / 强调描边对比 / **只给结束点**；
3. 失败写法（已实测）：给时间窗（`from 1.0 seconds to 4.0 seconds`）、台词 1.0s 才起、负面时间约束、中文字幕指令；
4. **档位不是硬规则**：每档都能要求出字幕，是否带字幕由用户/脚本需求决定；`quality` 档字准最稳（建议成片），`balanced` / `fast` 出字幕经自检门核对后按需升档重渲；
5. 口型安全 / 角色一致 / 环境一致 / 音频轨道六段式与旧格式同级，动作执行六段式略优 → 六段式可作为默认路径。
