...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ComfyUISettings() {
      ...
      // 策略是组级的：只覆盖它自己那组的档位，同能力其它组的选择保持不变
      const applyStrategy = (cap, strategy) =>
        saveTiers({ ...cfg, tiers: { ...(cfg.tiers || {}), [cap]: { ...((cfg.tiers || {})[cap] || {}), ...strategy.tiers } } }, `已应用策略「${strategy.label}」`)
      ...
      const tieredBlock = (c) => {
        ...
        // 策略是「某能力下**某一组** workflow 的一套实现」，因此按组渲染：
        //  - 单选按钮按组隔离（name 带组 id）——否则同能力多组时会串台，只能选中一个；
        //  - 应用某组策略只覆盖该组自己的档位，不覆盖同能力其它组的选择。
        const groupStrategyBlocks = groups.map((g) => {
          const gs = g.strategies || []
          if (!gs.length) return null
          const firstOfGroup = (tier) => (((g.tiers || {})[tier] || [])[0] || {}).id || null
          const effOf = (tier) => cur[tier] || firstOfGroup(tier)
          // 只比该策略自己的档位（= 该组的档位集合）：空配置下它应显示为已选中
          const matchOf = (s) => Object.keys(s.tiers || {}).every((t) => (effOf(t) || null) === (s.tiers[t] || null))
          const matched = gs.find(matchOf)
          const rows = gs.map((s) =>
            React.createElement('label', { key: s.id, style: { display: 'grid', gridTemplateColumns: 'auto minmax(0, 1fr)', gap: 6, alignItems: 'start', margin: '5px 0', cursor: 'pointer' } },
              React.createElement('input', { type: 'radio', name: 'strategy-' + c.capability + '-' + g.id, checked: Boolean(matched && matched.id === s.id), onChange: () => applyStrategy(c.capability, s), style: { marginTop: 3 } }),
              React.createElement('span', { style: { minWidth: 0 } },
                React.createElement('span', { style: { display: 'block' } }, s.label),
                React.createElement('span', { style: { display: 'block', ...noteStyle } },
                  (c.tiers || []).filter((t) => (s.tiers || {})[t]).map((t) => t + ' → ' + shortIdOf(s.tiers[t]) + (accelOf(s.tiers[t]) ? ' ⚡' : '')).join('　｜　')),
              ),
            ),
          )
          if (!matched) {
            rows.push(React.createElement('label', { key: '__custom', style: { display: 'grid', gridTemplateColumns: 'auto minmax(0, 1fr)', gap: 6, margin: '5px 0' } },
              React.createElement('input', { type: 'radio', name: 'strategy-' + c.capability + '-' + g.id, checked: true, readOnly: true, style: { marginTop: 3 } }),
              React.createElement('span', { style: noteStyle }, '自定义（该组逐档自由组合）'),
            ))
          }
          return React.createElement('div', { key: 'sg-' + g.id, style: { marginBottom: groups.length > 1 ? 6 : 0 } },
            groups.length > 1 ? React.createElement('div', { style: { ...noteStyle, margin: '2px 0' } }, g.displayName) : null,
            rows,
          )
        }).filter(Boolean)

        const tierRows = (c.tiers || []).map((tier) => {
        ...
        return React.createElement('div', { key: c.capability, style: { margin: '10px 0', padding: '10px 12px', border: '1px solid var(--dsw-alias-border-l1, #ffffff0f)', borderRadius: 8, minWidth: 0 } },
          ...
          React.createElement('div', { style: { ...small, margin: '2px 0 6px' } }, '策略＝该组一套实现（点选即写入配置）；也可逐档自由组合。'),
          groupStrategyBlocks,
