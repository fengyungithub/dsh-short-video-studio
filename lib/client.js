...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function svsVideoHeader(cur) {
      const vt = cur.videoTier || 'fast'
      const dims = svsDims(cur.ratio, vt === 'fast' ? 832 : 1344)
      let args = 'type=' + cur.pipeline + ' tier=' + vt + ' ratio=' + cur.ratio
    ...
    function svsSetHeader(inputActions, cur, draft) {
      ...
      const head = cur.mode === 'image' ? svsImageHeader(cur) : svsVideoHeader(cur)
    ...
    // ------------------------------------------------------------------
    const GEN_DEFAULT = { mode: 'chat', pipeline: 'r2v', videoTier: 'fast', imageTier: 'quality', ratio: '16:9', seconds: 124, refs: [], first: null, last: null, imgType: 't2i', count: 1, results: [] }
    ...
    function InnerGenBar(props) {
      ...
      if (mode === 'image') {
        ...
        return h('div', Object.assign({ className: 'svsg-inner' + (dragging ? ' svsg-dropon' : '') }, dropZone),
          ...
          h('div', { className: 'svsg-row' },
            ...
            seg([['t2i', '文生图'], ['i2i', '图生图']], st.imgType || 't2i', setImgType),
            isII ? null : h('span', { className: 'svsg-lbl' }, '比例'),
            isII ? null : sel(GEN_RATIOS.image.map((r) => [r, r]), st.ratio, (v) => { mutate({ ratio: v }); setTip(''); setError('') }),
            isII ? h('span', { className: 'svsg-lbl' }, '档位') : null,
            isII ? sel([['quality', 'quality'], ['fast', 'fast']], st.imageTier || 'quality', (v) => mutate({ imageTier: v })) : null,
            h('span', { className: 'svsg-lbl' }, '张数'),
            sel(imgCounts, String(st.count || 1), (v) => mutate({ count: Number(v) || 1 })),
      ...
      const dims = svsDims(st.ratio, (st.videoTier || 'fast') === 'fast' ? 832 : 1344)
      ...
      return h('div', Object.assign({ className: 'svsg-inner' + (dragging ? ' svsg-dropon' : '') }, dropZone),
        h('div', { className: 'svsg-row' },
          ...
          sel([['quality', 'quality'], ['fast', 'fast']], st.videoTier || 'fast', (v) => mutate({ videoTier: v })),
