  ...
  factory: (require) => {
    ...
    function ComfyUISettings() {
      ...
      const addStrategy = (cap) => {
        ...
        if (!Object.keys(tiers).length) { setMsg('至少给一个档位选实现（其余档位将跟随注册表首选）'); return }
      ...
      const tieredBlock = (c) => {
        ...
        const tierSummary = (s) => (c.tiers || []).map((t) => {
          const id = (s.tiers || {})[t]
          return t + ' → ' + (id ? shortIdOf(id) + (accelOf(id) ? ' ⚡' : '') : '默认')
        }).join('　｜　')
        const strategyRow = (s) =>
        ...
        const newStrategyForm = newStratCap === c.capability
          ? React.createElement('div', { style: { margin: '6px 0 2px', padding: '8px 10px', border: '1px dashed var(--dsw-alias-border-l2, #ffffff26)', borderRadius: 8 } },
              ...
              (c.tiers || []).map((tier) => {
                ...
                return React.createElement('div', { key: 'ns-' + tier, style: { display: 'grid', gridTemplateColumns: '72px minmax(0, 1fr)', gap: 10, alignItems: 'center', margin: '5px 0' } },
                  ...
                  React.createElement('select', { value: newStratTiers[tier] || '', onChange: (e) => setNewStratTiers({ ...newStratTiers, [tier]: e.target.value }), style: ctrlStyle },
                    React.createElement('option', { value: '' }, '跟随注册表首选'),
              ...
              React.createElement('div', { style: { display: 'flex', gap: 6, marginTop: 6 } },
                React.createElement('button', { onClick: () => addStrategy(c.capability), style: { ...btnStyle, padding: '3px 12px', fontSize: 12 } }, '保存并选用'),
                React.createElement('button', { onClick: () => { setNewStratCap(''); setNewStratName(''); setNewStratTiers({}) }, style: { ...btnStyle, padding: '3px 12px', fontSize: 12 } }, '取消'),
              ),
            )
          : null
        ...
        return React.createElement('div', { key: c.capability, style: { margin: '10px 0', padding: '10px 12px', border: '1px solid var(--dsw-alias-border-l1, #ffffff0f)', borderRadius: 8, minWidth: 0 } },
          ...
          strategyRows.length
            ? React.createElement('div', null,
                React.createElement('div', { style: { ...small, margin: '4px 0 2px' } }, '策略（单选；内置默认 = 跟随注册表首选，其余是你自己命名并组合的）'),
                strategyRows,
                React.createElement('div', { style: { margin: '4px 0 2px' } },
                  React.createElement('button', { onClick: () => { setNewStratCap(newStratCap === c.capability ? '' : c.capability); setNewStratName(''); setNewStratTiers({}) }, style: { ...btnStyle, padding: '2px 10px', fontSize: 12 } },
                    newStratCap === c.capability ? '取消新增' : '＋ 新增策略（命名 + 逐档组合）'),
                ),
              )
            : null,
          newStrategyForm,
