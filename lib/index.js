...
## 渠道交付（TUI / 飞书）
- 「画布」tab 只在 Web 可见。跑在 TUI / 飞书时用户看不到画布，交付阶段要主动把产物送达用户，而不是只报一句媒体路径。
- **飞书渠道会注册 send_file(path) 工具**：若当前会话的可用工具里有它，对要交付的产物（角色卡/场景卡/逐镜片段/拼接成片）逐一 send_file(相对路径)，相对路径取生成工具返回的 media 字段。群聊会弹审批卡，由用户在飞书确认，照常调用即可。
- **没有 send_file（Web / TUI）**：产物已在画布/工作区，无需推送；TUI 用户按工具返回的 absPath（绝对路径）自行打开。
- send_file 有单文件上限（默认 20 MiB）：成片或长片段超限时改用 fast 档 / 更短镜头 / 分镜分开发，或提示用户到画布取。
