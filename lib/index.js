...
async function runRender(ctx, opts) {
  ...
  return { ok: true, node, files, ...mediaDelivery(root, node.media) }
}
...
- **文本节点只进回复，不发文件**：简报/大纲/镜头表/分镜/交付清单等 text/table 节点，把要点写进助手回复即可，不要为纯文本调 send_file。
- **媒体节点先送达、后提问**：图片（角色卡/场景卡/铅笔分镜/抽帧）与视频（逐镜片段/转场镜/拼接成片）每个产物落画布后，若当前可用工具里有 send_file，**先逐一 send_file(生成工具返回的 media 相对路径) 送达，再出 ask_user_question 选项卡问下一步**。群聊会弹审批卡，由用户在飞书确认，照常调用即可。
- **没有 send_file（Web / TUI）**：产物已在画布/工作区，无需推送；TUI 用户按工具返回的 absPath 自行打开。
- send_file 有单文件上限（默认 20 MiB，可配）：成片或长片段超限时改用 fast 档 / 更短镜头 / 分镜分开发，或提示用户到画布取。
