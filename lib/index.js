...
- comfy_generate_video(prompt, mode?, ref_nodes?, first_frame_node?, last_frame_node?, width?, height?, length?, seed?, steps?, title?, group?, nodeId?)：MiniMax H3 音视频模型生成单镜头视频（带声音）。mode=quality(20步，成片) 或 fast(4步+Lightning LoRA，调试快5×+)。**分辨率由画布 aspectRatio 推导（fast/quality 均支持 16:9/9:16/1:1 等任意比例），不写死 832×480**；显式传 width/height 可覆盖。两种路径：a) 传 first_frame_node/last_frame_node → ImageToVideo 首/末帧串联（同场景续接，身份/环境由首帧继承，改善过渡）；b) 不传 → ref_nodes 传画布角色卡/场景卡节点 id 作参考绑定。length 为 24fps 帧数（124≈5s）。单镜头用它。
