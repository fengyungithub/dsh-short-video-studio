...
const H3_REF2V_IDS = [
  ...
  'minimax-h3-ref2v-pdd-balanced', 'minimax-h3-ref2v-pdd-balanced-sol',
]
const H3_I2V_IDS = [
  'minimax-h3-i2v-fast', 'minimax-h3-i2v-balanced', 'minimax-h3-i2v-balanced-sol',
  'minimax-h3-i2v-quality', 'minimax-h3-i2v-quality-sol',
  'minimax-h3-i2v-pdd-balanced', 'minimax-h3-i2v-pdd-balanced-sol',
]
...
function projectGroupStrategies(group) {
  ...
  for (const [tier, list] of Object.entries(group.tiers)) {
    if (list.some((m) => !m.accel)) hasStd = true
    if (list.some((m) => m.accel)) hasAcc = true
    std[tier] = (list.find((m) => !m.accel) || list[0]).id
    acc[tier] = (list.find((m) => m.accel) || list[0]).id
  }
  // 策略是**组级**的：id/label 带组信息，消费方无需靠嵌套位置推断它属于哪一组。
  // 标签按"组里到底有什么"派生，而不是无脑加后缀：组内只有一种实现（如 PDD 专用组）时，
  // 再加「（无加速）」只会误导——那时组的 displayName 本身就说明了它是什么。
  const name = group.displayName || group.id
  const entries = []
  if (hasStd) entries.push({ id: `${group.id}::standard`, group: group.id, label: hasAcc ? `${name}（无加速）` : name, tiers: std })
  const identical = TIERS.every((t) => (std[t] || null) === (acc[t] || null))
  if (hasAcc && !identical) entries.push({ id: `${group.id}::accel`, group: group.id, label: `${name}（有加速）`, tiers: acc })
  return entries
}
