...
function makeTools() {
  ...
  const concatTool = {
    ...
    output: {
      schema: {
        type: 'object',
        properties: { ok: { type: 'boolean' }, nodeId: { type: 'string' }, media: { type: 'string' }, backend: { type: 'string' }, error: { type: 'string' } },
      },
      render: (_args, value) => text(value.ok
    ...
    async execute(args, exec) {
      try {
        ...
        return { ok: true, nodeId: r.node.id, media: r.node.media, backend: r.backend, error: '' }
      } catch (error) {
        return { ok: false, nodeId: '', media: '', backend: '', error: error?.message || String(error) }
      }
    },
  }
  ...
  const assetToCanvasTool = {
    ...
    async execute(args, exec) {
      try {
        ...
        const project = await loadProject(root, sid)
        const group = args.group || DEFAULT_GROUP
        const node = {
          id: nodeId,
          kind: 'image',
  ...
  return [imageTool, videoTool, renderTool, concatTool, listWorkflowsTool, listNodesTool, writeNodeTool, getNodeTool, groupTool, reorderTool, stateTool, setStateTool, assetListTool, assetToCanvasTool]
}
...
- video_concat(nodes[], title?, group?, nodeId?)：把画布上的多个视频节点按给定顺序拼接成完整成片（含音轨），产物写回画布。后端自动选择：有 ffmpeg 走 ffmpeg（零重编码），否则走 ComfyUI 纯节点链路。只做硬切，不做溶解转场。
