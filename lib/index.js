...
3 角色卡（comfy_generate_image，group="character cards"，16:9，含正/侧/背三视图与 speaks_on_screen 标注）→ 选项卡锁定。生成前先 asset_list(type=character) 查库；命中复用则 asset_to_canvas(id=资产 id) 把卡片物化到画布（复用也要在画布可见），未命中才 comfy_generate_image 新卡。**新卡不自动入库**——生成后提示用户在画布点该卡的「入库」按钮人工登记（AI 不调用任何入库动作）。
