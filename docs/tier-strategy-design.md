...
**枚举是词汇表，不是每个组的档位清单**：一个能力可以有多个组（如 ref2v 的普通组 + PDD 组），
每个组可只提供其中一部分档（PDD 组目前只有 `balanced`），能力层的档位列表 = 各组并集。
...
- **标签按"组里到底有什么"派生，不无脑加后缀**（2026-09 实现）：组内**既有**非 accel 又有 accel → 两条 `（无加速）`/`（有加速）`；组内**只有一种**（例如 PDD 专用组里全是非 accel 实现）→ **只显示一条且不加任何后缀**，标题就是组的 `displayName`。否则会出现「MiniMax H3 参考生成视频 · PDD 蒸馏（无加速）」这种自相矛盾的叠词——组名本身已经说明了它是什么。
...
3. 未配置 → 该 capability 下**所有组**里，该档的候选按 `priority desc → 非 accel 优先 → id asc` 取首个
   （同一能力可以有多个组，因此这是**跨组**排序；把某个组整体设成 `priority < 0`
   即可让它"**可显式选中、但不参与隐式默认**"——PDD 组正是这么做的，它依赖第三方节点）
...
video.reference2video
  ...
  ( ) 自定义 ▾   （逐档下拉，按组分组，可跨组）
  ── 另一组（同能力、独立策略）：
  ( ) MiniMax H3 参考生成视频 · PDD 蒸馏（无加速）   ← 8 步拿到成片档画质（可选，需第三方节点）
  (•) MiniMax H3 参考生成视频 · PDD 蒸馏（有加速）      137s @1344×768 · 细节量≈成片档
...
- **PDD 是否再加档位**：目前每组只提供 `balanced`（nfe=8，已实测）。官方还允许 `nfe=4`（块长重排，未实测）与"两段式超分"（`MiniMaxH3AVLatentUpscaleBy`，节点包自带纯 resize，未实测）——需要更低成本时才补，补的话是**在 PDD 组内加档**，不动其它组。
