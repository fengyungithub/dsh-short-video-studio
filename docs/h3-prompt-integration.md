...
- **字幕**：已由 h3-prompt-writing 内嵌 detailed_description（§5.4 定稿），片型侧不再追加末尾中文指令；仅自由格式兜底路径保留；
- **负向意图**（一句，写在文末）。

### 5.4 实验结论（2025-08-31 已实测，A/B 完成）

同一分镜（《小狐狸》S01 复刻，5s、seed=20240901、同 ref_nodes）六变体对比，完整矩阵见画布「A/B 实验结论」节点与 `references/studio-mapping.md` §5：

| 变体 | 档位 | 字幕写法 | 结果 |
|---|---|---|---|
| 旧自由格式 | fast | 末尾中文指令 | ✅ 正确 |
| 六段式 | fast/quality | 末尾中文指令 | ❌ 不烧 |
| 六段式 | fast | 内嵌英文 on-screen 声明 | ⚠️ 烧但错字 |
| **六段式** | **quality** | **内嵌英文 on-screen 声明** | ✅ **正确（逐字一致）** |

**定稿规则**：

1. **字幕**：六段式下必须按官方 base-en.txt §4.5 规范，以英文双引号 on-screen text 声明**内嵌 `detailed_description` 对白处**；中文措辞指令（任何位置）不生效。片型 skill 不再追加末尾中文指令（仅自由格式兜底保留）。
2. **档位**：dialogue 镜成片用 quality 保证字准；fast 仅调试。
3. **无回归**：口型安全 / 角色一致 / 环境一致 / 音频（AAC 32kHz 立体声）六段式与旧格式同级；动作执行六段式略优 → 六段式作为默认路径。
...
> 3. 在 h3-prompt-writing 输出之后拼接片型级后缀：风格锁 → 负向意图（字幕已内嵌于 h3-prompt-writing 输出；仅自由格式兜底路径追加末尾中文字幕指令）。
