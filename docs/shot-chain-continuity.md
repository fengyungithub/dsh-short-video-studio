...
| 一条链内**首遍尺寸必须一致**（= 采样器实际跑的尺寸，不是交付尺寸） | 报首遍尺寸不符：链上传递的是**首遍 latent**，latent 无法缩放。要么让两镜用首遍尺寸相同的实现（**交付尺寸可以不同**），要么从这里另起一链 |
...
* **长时间链**：作者提示音频在长链上会逐渐变闷、**首遍尺寸锁死整条链**（交付尺寸不锁——放大发生在链式存档之后）；本插件未做链长限制（本轮最长只验到 3 镜）。
...
## 附 F：学习式放大 × 链式续接（`-ctx-*-2k` 族，fast 档实测 · 2026-09-23）

**要回答的问题**：>2K 的交付靠**两阶段放大**（首遍原生尺寸 → 视频 latent ×2 → 低噪声二遍重建），
它和二遍都在链式存档**之后**——那么「放大」与「续接」能不能叠用？

**接线事实（读图得出，不是推断）**：ctx 图的 `MiniMaxH3MotionContextSaveLatent`（node 22）读的是
**node 10 = 首遍采样器的输出**，放大节点在其后。所以：

* 链上流动的永远是**首遍 latent** ⇒ 放大实现**可以**续接，判据落在**首遍尺寸**上；
* **交付尺寸不参与**续接 ⇒ 一条链里可以有「首遍相同、交付不同」的镜（例如 `-ctx-quality` 1344×768 接 `-ctx-quality-2k` 2688×1536）；
* 反过来，首遍不同（fast 832×480 ↔ quality 1344×768）**拒跑**——latent 不能缩放。

**实测（2026-09-23，`scripts/e2e-chain-continuity.mjs`，交付 1664×960 / 首遍 832×480）**：

| 镜 | 实现 | 交付 | 帧数（请求/采样/交付） | 链序号 |
|---|---|---|---|---|
| S01 起链 | `minimax-h3-ref2v-ctx-fast-2k` | 1664×960 | 56 / 56 / 56 | load 0 → save 156 |
| S02 续接 | 同上 + `continuity_from=S01` | 1664×960 | 56 / 90 / **68** | load 156 → save 157 |
| S02 对照（同实现·**不**续接） | 同上，不传 `continuity_from` | 1664×960 | 68 / 73 / 73 | 另起一链 save 158 |
| S02 对照（非链式实现） | `minimax-h3-ref2v-fast` | 832×480 | 68 / 73 / 73 | 无（用于验证"非链式上一镜被拒") |

接缝指标（`measure_seam.py`，与附 B 同一口径）：

| 指标 | 续接 | 对照（同实现·不续接） |
|---|---|---|
| 画面接缝 MAD（末帧↔首帧） | **13.33** /255 | **79.57** /255 |
| 音频宽带相关（100 ms 窗） | **0.898** | 0.391 |
| 音频包络相关 | −0.231 | −0.025 |
| 响度台阶 | **+0.68 dB** | **−6.34 dB** |

对照附 B 的非放大族（2.6–7.0 vs 32–68，响度台阶 +0.09…−0.77 dB vs −8.8…−24 dB）：

1. **放大族接缝绝对值略糙**（13.33 vs 2.6–7.0）——两阶段重建在接缝附近本来就多一次不确定性；
   但相对"另起一镜"仍紧 **6 倍**，音频也确实是"接着走"（+0.68 dB vs −6.34 dB）。
2. **结论**：>2K 交付与链式续接**可以叠用**，用 `-ctx-*-2k` 族即可；不需要为了续接放弃 2K。
3. **代价**：fast 档 56 帧一镜 114.0s（起链）/ 246.3s（续接，多采到 90 帧）——续接镜贵在采样帧数，
   不在放大本身。quality 档同尺寸预估 ~1900s（二遍 6 步 20 步首遍），要成片再上。

**复现**：

```bash
node scripts/e2e-chain-continuity.mjs --tier=fast --workflow-suffix=-2k \
  --plain=minimax-h3-ref2v-fast --control-workflow=minimax-h3-ref2v-ctx-fast-2k --length=56
# 接缝量化：把三条产物送进容器跑 e2e-out/ctx-test/measure_seam.py（宿主无 ffmpeg，容器自带 av+numpy）
```

**顺带的契约修正**：`docs/` 与 UI 里此前"两阶段放大与链式续接不能叠用"的说法是**错的**，
正确口径是「**同一条链内首遍尺寸一致**」。runner 已按此判据实现（`chainGraphMismatch`，纯函数，
`scripts/smoke-hires-lock.mjs` §9 有 GPU-free 单测）。
